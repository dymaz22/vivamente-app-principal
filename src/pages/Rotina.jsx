...
// Componente para item de tarefa diária (agora sem duplicata)
...
const Rotina = () => {
  ...
  // CORREÇÃO: Força uma nova busca dos dados toda vez que a página de Rotina for exibida.
  useEffect(() => {
    getDailyRoutine();
  }, []);

  // Telas de loading e erro (sem mudanças)
  ...
  // Lógica de cálculo de progresso e handlers
  ...
  const handleTaskComplete = async (taskType) => { if (dailyRoutine?.completions?.[taskType]) return; await markTaskAsCompleted(taskType); };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f23] p-4">
      <div className="container mx-auto max-w-md">
        ...
        <div className="space-y-4"><h2 className="text-lg font-semibold text-white mb-4">Atividades do Dia</h2>
          <DailyTaskItem task={dailyRoutine?.lesson} isCompleted={dailyRoutine?.completions?.lesson} onToggleComplete={() => handleTaskComplete('lesson')} icon={BookOpen} type="lesson" />
          <DailyTaskItem task={dailyRoutine?.quiz} isCompleted={dailyRoutine?.completions?.quiz} onToggleComplete={() => handleTaskComplete('quiz')} icon={Brain} type="quiz" />
          <DailyTaskItem task={dailyRoutine?.task} isCompleted={dailyRoutine?.completions?.task} onToggleComplete={() => handleTaskComplete('task')} icon={CheckCircle} type="task" />
        </div>
        <div className="h-20" />
      </div>
    </div>
