...
const MoodStep3_Context = () => {
  ...
  const locationOptions = [
    'Casa', 'Trabalho', 'Escola', 'Rua', 'Parque', 'Academia', 'Restaurante', 'Transporte', 'Outro'
  ];

  const socialContextOptions = [
    'Sozinho(a)', 'Com família', 'Com amigos', 'Com colegas', 'Com parceiro(a)', 'Em grupo', 'Com estranhos'
  ];

  const activityOptions = [
    'Trabalhando', 'Estudando', 'Exercitando', 'Comendo', 'Descansando', 'Socializando', 
    'Viajando', 'Assistindo TV', 'Lendo', 'Meditando', 'Outro'
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const moodData = {
        ...
        social_context: selectedSocialContext,
        activity: selectedActivity,
        sentiment_ids: selectedSentiments || []
      };

      const result = await addMoodLog(moodData);
      
      if (result.success) {
        // Navegar para tela de sucesso ou voltar para rotina
        navigate('/rotina', { 
          state: { 
            message: 'Humor registrado com sucesso! Obrigado por compartilhar.' 
          } 
        });
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f23] p-4">
      <div className="container mx-auto max-w-md">
        ...
        <Card className="bg-card/30 backdrop-blur-sm border-border/50 mb-6">
          ...
          <CardContent>
            <Textarea
              placeholder="Escreva sobre como você está se sentindo, o que aconteceu hoje, ou qualquer coisa que queira registrar..."
              value={notes}
