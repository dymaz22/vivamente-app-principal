...
const MoodStep3_Context = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Bloco de sucesso: se o registro funcionar, navega para a tela de sucesso
      if (result.success) {
        navigate('/rotina/humor/sucesso'); // <-- MUDANÇA CORRETA É AQUI
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f23] p-4">
      <div className="container mx-auto max-w-md">
        ...
        <Card className="bg-card/30 backdrop-blur-sm border-border/50 mb-6">
          <CardHeader><CardTitle className="text-white flex items-center gap-2"><Activity className="w-5 h-5" />O que lhe vai na cabeça?</CardTitle><CardDescription className="text-white/70">Compartilhe seus pensamentos (opcional)</CardDescription></CardHeader>
          <CardContent>
            <Textarea placeholder="Escreva sobre como você está se sentindo..." value={notes} onChange={(e) => setNotes(e.target.value)} className="bg-card/20 border-border/30 text-white placeholder:text-white/50 min-h-[100px]" maxLength={500} />
        ...
        <Card className="bg-card/30 backdrop-blur-sm border-border/50 mb-6">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {locationOptions.map((location) => (<Badge key={location} variant={selectedLocation === location ? "default" : "secondary"} className={`cursor-pointer transition-all ${selectedLocation === location ? 'bg-primary text-primary-foreground' : 'bg-card/40 text-white hover:bg-card/60'}`} onClick={() => setSelectedLocation(location === selectedLocation ? '' : location)}>{location}</Badge>))}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card/30 backdrop-blur-sm border-border/50 mb-6">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {socialContextOptions.map((context) => (<Badge key={context} variant={selectedSocialContext === context ? "default" : "secondary"} className={`cursor-pointer transition-all ${selectedSocialContext === context ? 'bg-primary text-primary-foreground' : 'bg-card/40 text-white hover:bg-card/60'}`} onClick={() => setSelectedSocialContext(context === selectedSocialContext ? '' : context)}>{context}</Badge>))}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card/30 backdrop-blur-sm border-border/50 mb-8">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {activityOptions.map((activity) => (<Badge key={activity} variant={selectedActivity === activity ? "default" : "secondary"} className={`cursor-pointer transition-all ${selectedActivity === activity ? 'bg-primary text-primary-foreground' : 'bg-card/40 text-white hover:bg-card/60'}`} onClick={() => setSelectedActivity(activity === selectedActivity ? '' : activity)}>{activity}</Badge>))}
            </div>
          </CardContent>
        </Card>
        <div className="flex justify-between items-center">
          ...
          <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-primary hover:bg-primary/90 disabled:opacity-50">
            {isSubmitting ? (<><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2" />Salvando...</>) : ('Finalizar')}
          </Button>
        </div>
        <div className="h-20" />
      </div>
