...
const Licao = () => {
  ...
  // Inicializar rating quando os dados carregarem
  React.useEffect(() => {
  ...
  const handleComplete = async () => {
    ...
    try {
      const result = await markLessonAsComplete(parseInt(id));
      if (result.success) {
        refetch(); // Recarregar dados da lição
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f23] relative">
        ...
        <div className="relative z-10 container mx-auto px-4 py-8 max-w-4xl">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <Skeleton className="h-8 w-64 mb-2 bg-card/30" />
          ...
          {/* Video Skeleton */}
          <div className="mb-8">
            <Skeleton className="aspect-video w-full bg-card/30 rounded-2xl" />
          </div>
          
          {/* Content Skeleton */}
          <div className="mb-8">
            <Skeleton className="h-32 w-full bg-card/30 rounded-2xl" />
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f23] relative flex items-center justify-center">
        ...
        <div className="relative z-10 text-center">
          <h2 className="text-2xl font-bold text-white mb-4">Erro ao carregar lição</h2>
  ...
  if (!lessonDetails) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f23] relative flex items-center justify-center">
        <PetalsAnimation />
        <div className="relative z-10 text-center">
          <h2 className="text-2xl font-bold text-white mb-4">Lição não encontrada</h2>
          <Button onClick={() => navigate(-1)} variant="outline">
            Voltar
          </Button>
        </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-4xl">
          ...
          <div className="mb-8">
            <div className="bg-card/30 backdrop-blur-sm border border-border/50 rounded-2xl p-6">
              <div className="aspect-video bg-black rounded-xl mb-4 flex items-center justify-center">
                <iframe
                  src={lessonDetails.video_url.replace('watch?v=', 'embed/')}
                  title={lessonDetails.title_pt}
              ...
              <div>
                ...
                <p className="text-white/60">
                  {isCompleted 
                    ...
                    : 'Clique para marcar esta lição como concluída.'
                  }
                </p>
              </div>
              <Button
        ...
        <div className="mb-8">
          <div className="bg-card/30 backdrop-blur-sm border border-border/50 rounded-2xl p-6">
            ...
            <div className="mb-6">
              <Textarea
                placeholder="Adicione seu comentário sobre esta lição..."
                value={newComment}
                  ...
                  <div key={comment.id} className="border-b border-border/30 pb-4 last:border-b-0">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-white">
                        {comment.user_id === 'user1' ? 'Você' : `Usuário ${comment.user_id}`}
                      </span>
